...
class TextMemoryActivitiesMixin:
    ...
    @activity(
        config={
            ...
            "schema": Schema(
                {"memory_name": str, "artifact_namespace": str, "text": str}
            ),
        }
    )
    def insert(self, params: dict) -> InfoArtifact | ErrorArtifact:
        ...
        if memory:
            memory.query_engine.upsert_text_artifact(
                TextArtifact(text), artifact_namespace
            )
    ...
    def extract_csv_rows(self, params: dict) -> ListArtifact | ErrorArtifact:
        ...
        if memory:
            return memory.csv_extraction_engine.extract(
                memory.load_artifacts(artifact_namespace), column_names
            )
        else:
            return ErrorArtifact("memory not found")
    ...
    def extract_json_objects(
        ...
        if memory:
            return memory.json_extraction_engine.extract(
                memory.load_artifacts(artifact_namespace), json_schema
            )
        else:
            return ErrorArtifact("memory not found")
