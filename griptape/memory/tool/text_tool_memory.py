...
class TextToolMemory(BaseToolMemory, TextMemoryActivitiesMixin):
    ...
    csv_extraction_engine: CsvExtractionEngine = field(
        kw_only=True, default=Factory(lambda: CsvExtractionEngine())
    )
    json_extraction_engine: JsonExtractionEngine = field(
        kw_only=True, default=Factory(lambda: JsonExtractionEngine())
    )
    ...
    def process_output(
        ...
        if isinstance(output_artifact, TextArtifact):
            ...
            self.query_engine.upsert_text_artifact(
                output_artifact, namespace=namespace
            )
            ...
            if output_artifact.has_items():
                ...
                self.query_engine.upsert_text_artifact(
                    TextArtifact(output_artifact.to_text()), namespace
                )
            else:
                namespace = None
        else:
        ...
        if namespace:
            ...
            logging.info(
                f"Output of {tool_name}.{activity_name} can't be processed by memory {self.name}"
            )
